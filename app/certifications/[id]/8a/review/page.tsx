...
export default function EightAReviewPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    try {
      ...
      try {
        ...
        if (savedReviews && savedReviews.length > 0) {
          ...
          if (stored) { try { const p = JSON.parse(stored); setReview({ version: "v1", ...p.review }); setReviewHistory(p.history || []); } catch {} }
        }
      } catch {
        const stored = localStorage.getItem(`govcert-review-${certId}`);
        if (stored) { try { const p = JSON.parse(stored); setReview({ version: "v1", ...p.review }); setReviewHistory(p.history || []); } catch {} }
  ...
  async function resolveIssue(issueKey: string, resolved: boolean, dismissed?: boolean) {
    ...
    try {
      ...
      if (dismissed) { result.resolvedIssues[issueKey] = { ...result.resolvedIssues[issueKey], dismissed: true }; }
  ...
  function renderFindingActions(key: string, findingText: string, sectionId: string, sectionLabel: string) {
    ...
    return (
      <div style={{ display: "flex", gap: 4, flexShrink: 0, marginTop: 6 }}>
        {!isResolved && (
          <>
            <button onClick={() => setGuidedFix({ isOpen: true, issueKey: key, issueText: findingText, sectionId, sectionLabel })}
            ...
            <button onClick={() => {}} style={{ padding: "4px 10px", borderRadius: 5, fontSize: 11, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" as const, background: "transparent", color: "var(--ink3)", border: "1px solid var(--border)" }}>
              Fix manually
            </button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
            <div>
              ...
              <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 36, color: "#fff", fontWeight: 400, marginBottom: 6 }}>GovCert Application Review</h1>
              <p style={{ fontSize: 14, color: "rgba(255,255,255,.45)", maxWidth: 500 }}>AI-powered analysis of your application against SBA regulatory standards and current approval patterns.</p>
            </div>
            <button onClick={runReview} disabled={analyzing}
              style={{ padding: "14px 32px", background: analyzing ? "var(--ink4)" : "linear-gradient(135deg, #C89B3C 0%, #E8B84B 100%)", border: "none", borderRadius: "var(--r)", fontSize: 15, fontWeight: 600, color: "#fff", cursor: analyzing ? "wait" : "pointer", boxShadow: analyzing ? "none" : "0 4px 24px rgba(200,155,60,.4)" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 48px" }}>
        {error && (
          <div style={{ background: "var(--red-bg)", border: "1px solid var(--red-b)", borderRadius: "var(--r)", padding: "12px 16px", marginBottom: 20, fontSize: 13, color: "var(--red)" }}>{error}</div>
        )}
        ...
        {!review && !analyzing && (
          <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "var(--rl)", padding: "48px", textAlign: "center" as const, boxShadow: "var(--shadow)" }}>
            ...
            <p style={{ fontSize: 14, color: "var(--ink3)", maxWidth: 480, margin: "0 auto 24px", lineHeight: 1.6 }}>GovCert will analyze every section of your 8(a) application against SBA regulations, identify gaps, score your readiness, and provide specific recommendations.</p>
            <button onClick={runReview} style={{ padding: "14px 40px", background: "var(--gold)", border: "none", borderRadius: "var(--r)", fontSize: 16, fontWeight: 600, color: "#fff", cursor: "pointer", boxShadow: "0 4px 24px rgba(200,155,60,.4)" }}>Start Review</button>
          </div>
        )}
        ...
        {review && !analyzing && v2 && spe && readiness && (<>
          <div style={{ background: "rgba(99,102,241,.04)", border: "1px solid rgba(99,102,241,.15)", borderRadius: "var(--rl)", padding: "16px 20px", marginBottom: 24, fontSize: 12, color: "var(--ink3)", lineHeight: 1.7 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 6 }}>This review is generated by AI and may contain inaccuracies. Use the "Not an issue" button to dismiss incorrect findings. This does not constitute legal advice and does not guarantee approval. GovCert is not affiliated with the SBA.</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "var(--rl)", padding: "28px 32px", marginBottom: 24, boxShadow: "var(--shadow)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 20 }}>
              {badge && (
                <div style={{ padding: "10px 28px", borderRadius: 8, fontSize: 20, fontWeight: 700, color: badge.color, background: badge.bg, letterSpacing: ".02em" }}>{badge.label}</div>
              )}
              {displayScore != null && (
            ...
            {readiness.summary && (
              <div style={{ fontSize: 13, color: "var(--ink2)", lineHeight: 1.7, padding: "12px 16px", background: "var(--cream)", borderRadius: 8, border: "1px solid var(--border)" }}>{readiness.summary}</div>
            )}
          </div>

          {/* Submission Package Items */}
          <div style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: ".1em", color: "var(--gold)", marginBottom: 14 }}>Submission Package Evaluation</div>
          {(spe.items || []).map((item: any) => {
            ...
            return (
              <div key={item.stepId} style={{ background: "#fff", border: `1px solid ${sc.border}`, borderLeft: `4px solid ${sc.color}`, borderRadius: "var(--rl)", marginBottom: 12, boxShadow: "var(--shadow)", overflow: "hidden" }}>
                <div onClick={() => toggleSet(setExpandedItems, item.stepId)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 22px", cursor: "pointer", userSelect: "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: "var(--navy)" }}>{item.label}</span>
                    <span style={{ padding: "2px 10px", borderRadius: 100, fontSize: 11, fontWeight: 600, background: sc.bg, color: sc.color, border: `1px solid ${sc.border}` }}>{sc.label}</span>
                    {findingCount > 0 && <span style={{ fontSize: 11, color: "var(--ink4)" }}>{findingCount} finding{findingCount !== 1 ? "s" : ""}</span>}
                  </div>
                      ...
                      return (
                        <div key={f.id} style={{ padding: "14px 16px", borderRadius: 8, background: isResolved ? "rgba(5,150,105,.04)" : sev.bg, border: `1px solid ${isResolved ? "rgba(5,150,105,.2)" : sev.color}20` }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                            <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 700, color: "#fff", background: sev.color, textTransform: "uppercase" as const, flexShrink: 0 }}>{f.severity}</span>
                            <div style={{ flex: 1 }}>
                            ...
                            {f.remediationType && (
                              <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 500, color: "var(--ink3)", background: "var(--cream2)", border: "1px solid var(--border)", flexShrink: 0 }}>{REMEDIATION_LABELS[f.remediationType] || f.remediationType}</span>
                            )}
                          </div>
                          <div onClick={() => toggleSet(setExpandedFindings, key)} style={{ fontSize: 11, color: "var(--gold)", cursor: "pointer", marginTop: 8, fontWeight: 500 }}>{isDetailOpen ? "Hide details" : "Show details"}</div>
                          {isDetailOpen && (
                            <div style={{ marginTop: 8 }}>
                              ...
                              {f.coLikelyAsk && (
                                <div style={{ padding: "12px 14px", borderRadius: 6, background: "#0B1929", marginBottom: 10 }}>
                                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--gold)", textTransform: "uppercase" as const, letterSpacing: ".08em", marginBottom: 4 }}>Contracting Officer Will Likely Ask</div>
                                  <div style={{ fontSize: 12.5, color: "rgba(255,255,255,.85)", lineHeight: 1.6, fontStyle: "italic" }}>&ldquo;{f.coLikelyAsk}&rdquo;</div>
                                </div>
                              )}
                              ...
                              {f.sourceRef && <div style={{ fontSize: 11, color: "var(--ink4)", fontStyle: "italic" }}>Source: {f.sourceRef}</div>}
                            </div>
                          )}
                ...
                {isExpanded && (!item.findings || item.findings.length === 0) && (
                  <div style={{ padding: "0 22px 18px", fontSize: 13, color: "var(--green)" }}>No findings for this item.</div>
                )}
              </div>
            );
          })}
          ...
          {spe.clarificationReadinessBrief && (
            <div style={{ marginTop: 28, marginBottom: 24 }}>
              <div onClick={() => setExpandedCRB(!expandedCRB)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 24px", background: "#0B1929", borderRadius: expandedCRB ? "var(--rl) var(--rl) 0 0" : "var(--rl)", cursor: "pointer", userSelect: "none" }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 600, color: "#fff", fontFamily: "'Cormorant Garamond', serif" }}>Clarification Readiness Brief</div>
              ...
              {expandedCRB && (
                <div style={{ background: "#fff", border: "1px solid var(--border)", borderTop: "none", borderRadius: "0 0 var(--rl) var(--rl)", padding: "20px 24px" }}>
                  <p style={{ fontSize: 13, color: "var(--ink3)", lineHeight: 1.6, marginBottom: 16 }}>This brief prepares you for the most common clarification requests. Each section covers a defense area with narrative context and per-item breakdowns.</p>
                  {Object.entries(CRB_SECTION_LABELS).map(([key, label]) => {
                    ...
                    return (
                      <div key={key} style={{ marginBottom: 10, border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
                        <div onClick={() => toggleSet(setExpandedCRBSections, key)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", background: "var(--cream)", cursor: "pointer", userSelect: "none" }}>
                        ...
                        {isOpen && (
                          <div style={{ padding: "16px 18px" }}>
                            {section.narrative && (
                              <div style={{ fontSize: 13, color: "var(--ink2)", lineHeight: 1.7, marginBottom: 12, padding: "12px 14px", background: "rgba(200,155,60,.04)", border: "1px solid rgba(200,155,60,.12)", borderRadius: 6 }}>{section.narrative}</div>
                            )}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 28, padding: "20px 24px", background: "var(--navy)", borderRadius: "var(--rl)" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {reviewId && (
                <a href={`${API_URL}/api/applications/ai/reviews/${reviewId}/docx`} target="_blank" rel="noopener noreferrer"
              ...
              <button onClick={runReview} disabled={analyzing} style={{ padding: "10px 24px", background: "var(--gold)", border: "none", borderRadius: "var(--r)", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer" }}>Re-Run Review</button>
              <a href={`/certifications/${certId}/8a/submit`} style={{ padding: "10px 24px", background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.15)", borderRadius: "var(--r)", color: "var(--gold2)", fontSize: 13, fontWeight: 500, textDecoration: "none" }}>Proceed to Submit</a>
            </div>
          </div>
          <div style={{ marginTop: 20, padding: "12px 16px", background: "var(--cream2)", borderRadius: "var(--r)", fontSize: 11, color: "var(--ink4)", lineHeight: 1.6 }}>
            <strong>Disclaimer:</strong> This AI-generated review does not constitute legal advice, does not guarantee approval, and cannot predict agency decisions. GovCert is not affiliated with the SBA or any federal agency. Always consult a government contracting professional before submission.
          </div>
        </>)}

        {/* ═══ V1 RESULTS (backward compat) ═══ */}
        {review && !analyzing && !v2 && (<>
          <div style={{ background: "rgba(99,102,241,.04)", border: "1px solid rgba(99,102,241,.15)", borderRadius: "var(--rl)", padding: "16px 20px", marginBottom: 24, fontSize: 12, color: "var(--ink3)", lineHeight: 1.7 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ fontSize: 18, flexShrink: 0 }}>&#9888;&#65039;</span>
              <div>
                <strong style={{ color: "var(--navy)" }}>AI-Generated Review — Use Professional Judgment</strong>
                <div style={{ marginTop: 6 }}>This review is generated by AI and may contain inaccuracies. Use the "Not an issue" button to dismiss incorrect findings. This does not constitute legal advice. GovCert is not affiliated with the SBA.</div>
              </div>
            </div>
          </div>
              ...
              <div key={section.id} style={{ background: "#fff", border: `1px solid ${cfg.border}`, borderLeft: `4px solid ${cfg.color}`, borderRadius: "var(--rl)", padding: "22px 26px", marginBottom: 14, boxShadow: "var(--shadow)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    {sectionLink && (
                      <a href={`/certifications/${certId}/${sectionLink}`} style={{ padding: "6px 14px", background: "var(--navy)", border: "none", borderRadius: "var(--r)", fontSize: 11, fontWeight: 500, color: "var(--gold2)", textDecoration: "none", whiteSpace: "nowrap" as const }}>Go to Section</a>
                    )}
                  </div>
                </div>
                      ...
                      return (
                        <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6, fontSize: 12.5, lineHeight: 1.5, padding: "6px 8px", borderRadius: 5, background: isResolved ? (resolvedIssues[key]?.dismissed ? "rgba(150,150,150,.06)" : "rgba(5,150,105,.06)") : undefined }}>
                          ...
                          <div style={{ flex: 1, color: isResolved ? (resolvedIssues[key]?.dismissed ? "#888" : "#065F46") : "var(--ink2)", textDecoration: isResolved ? "line-through" : undefined }}>
                            {imp}{resolvedIssues[key]?.dismissed && <span style={{ fontStyle: "italic", marginLeft: 6 }}>(dismissed)</span>}
                          </div>
                          <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                            {!isResolved && (
                              <>
                                <button onClick={() => setGuidedFix({ isOpen: true, issueKey: key, issueText: imp, sectionId: section.id, sectionLabel: section.label })}
                                  style={{ padding: "3px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600, cursor: "pointer", color: "#fff", background: "var(--gold)", border: "none" }}>Fix with AI</button>
                                <button disabled={resolvingKey === key} onClick={() => resolveIssue(key, true, true)}
                                  style={{ padding: "3px 8px", borderRadius: 4, fontSize: 10, fontWeight: 500, cursor: "pointer", background: "transparent", color: "#999", border: "1px solid #ddd" }}>Not an issue</button>
                              </>
                            )}
                            <button disabled={resolvingKey === key} onClick={() => resolveIssue(key, !isResolved)}
