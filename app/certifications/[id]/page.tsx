  ...
  const gsaSections = [
    {
      ...
      chars: (() => { try { const p = JSON.parse(app?.narrativeCorp || "{}"); const n = p.narratives || p; const keys = ["overview","capabilities","employees","org_controls","resources","past_projects","marketing","subcontractors"]; return keys.reduce((sum: number, k: string) => sum + ((n[k] || "").length), 0); } catch { return app?.narrativeCorp?.length || 0; } })(),
      charLimit: 10000,
    },
    {
      id: "qcp",
      ...
      chars: (() => { try { const p = JSON.parse(app?.narrativeQCP || "{}"); const n = p.answers || p; const keys = ["overview","supervision","personnel","subcontractors","corrective","urgent"]; return keys.reduce((sum: number, k: string) => sum + ((n[k] || "").length), 0); } catch { return app?.narrativeQCP?.length || 0; } })(),
      charLimit: 10000,
    },
    {
      id: "past-performance",
      label: "Past Performance & Project Experience",
      desc: `3 references + ${selectedSINs.length} SIN narrative${selectedSINs.length !== 1 ? "s" : ""} (10,000 chars each)`,
      icon: "⭐",
    ...
    {
      id: "negotiator-letter",
      label: "Authorized Negotiator Letter",
      desc: "Designate who can negotiate and sign the GSA contract",
      icon: "✍️",
      href: `/certifications/${certId}/negotiator-letter`,
      complete: (() => { try { const e = JSON.parse(app?.eofferData || "{}"); return !!e.negotiatorLetter; } catch { return false; } })(),
      chars: null,
      charLimit: null,
    },
    {
      id: "subcontracting-plan",
      label: "Subcontracting Plan",
      desc: (() => { try { const e = JSON.parse(app?.eofferData || "{}"); return e.clauseSmallBiz === "Small Business" ? "Small Business Exempt — no plan required" : "FAR 52.219-9 compliant plan for Other Than Small Business"; } catch { return "Required if Other Than Small Business with >$750K expected sales"; } })(),
      icon: "🤝",
      href: `/certifications/${certId}/subcontracting-plan`,
      complete: (() => { try { const e = JSON.parse(app?.eofferData || "{}"); return e.clauseSmallBiz === "Small Business" || !!e.subcontractingPlan; } catch { return false; } })(),
      chars: null,
      charLimit: null,
    },
  ];
