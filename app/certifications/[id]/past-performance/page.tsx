...
export default function PastPerformancePage({ params }: { params: { id: string } }) {
  ...
  const cparsUploadTarget = useRef<string | null>(null);
  ...
  async function saveContract(contract: typeof EMPTY_CONTRACT, index: number) {
    ...
    try {
      ...
      if (contract.id) {
        saved = await apiRequest(`/api/applications/${cert.application.id}/past-performance/${contract.id}`, {
          method: "PUT",
          body: JSON.stringify(payload)
        });
      } else {
        saved = await apiRequest(`/api/applications/${cert.application.id}/past-performance`, {
          method: "POST",
          body: JSON.stringify(payload)
        });
      }
      ...
      updated[index] = { ...contract, id: saved.id };
  ...
  async function addNewContract() {
    ...
    setSaving(true);
    try {
      const saved = await apiRequest(`/api/applications/${cert.application.id}/past-performance`, {
        method: "POST",
        body: JSON.stringify({
    ...
    try {
      const data = await apiRequest("/api/applications/ai/draft", {
        ...
        body: JSON.stringify({
          ...
          prompt: `Write a GSA MAS Relevant Project Experience narrative for this contract. Describe the SIN-relevant work performed, results achieved, methodology used, compliance with applicable regulations, and how this work aligns with the company's proposed GSA Schedule offering.`,
          context: {
...
Scope of Work: ${contract.sowDescription}`,
      ...
      setContracts(updated);
      if (contract.id) await saveContract(updated[index], index);
    } catch (err) { console.error(err); }
  ...
  async function handleCPARSUpload(file: File, index: number) {
    ...
    try {
      // In a full implementation this uploads to R2
      // For now mark as uploaded and save
      const updated = [...contracts];
      updated[index] = { ...contracts[index], cparsUploaded: true };
      setContracts(updated);
      if (updated[index].id) await saveContract(updated[index], index);
    } catch (err) { console.error(err); }
    finally { setUploadingCPARS(null); }
  }
  ...
  const appId = cert?.application?.id;
      ...
      <div style={{ width: 240, background: "var(--navy)", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
        ...
        <div style={{ padding: "16px 12px", flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ margin: "8px 9px 16px", padding: "12px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: "var(--r)" }}>
            ...
            {sentRefs > 0 && (
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", marginTop: 6 }}>{sentRefs} PPQ{sentRefs !== 1 ? "s" : ""} awaiting response</div>
            )}
          </div>
          ...
          {contracts.map((c, i) => (
            <div key={i} onClick={() => setExpandedContract(expandedContract === i ? null : i)}
              ...
              <div style={{ width: 16, height: 16, borderRadius: "50%", background: c.cparsUploaded || c.ppqStatus === "COMPLETED" ? "var(--green)" : c.ppqStatus === "SENT" || c.ppqStatus === "OPENED" ? "var(--blue,#1A3F7A)" : "rgba(255,255,255,.1)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#fff" }}>
        ...
        <div style={{ padding: "40px 48px", maxWidth: 900 }}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--ink3)" }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", background: i < completedRefs ? "var(--green)" : i < completedRefs + sentRefs ? "var(--blue,#1A3F7A)" : "var(--cream2)", border: `2px solid ${i < completedRefs ? "var(--green)" : i < completedRefs + sentRefs ? "var(--blue,#1A3F7A)" : "var(--border2)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#fff" }}>
          ...
          {addingContract && (
            <div style={{ background: "#fff", border: "2px solid var(--gold)", borderRadius: "var(--rl)", padding: "28px", marginBottom: 20, boxShadow: "var(--shadow-lg)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <button onClick={() => setAddingContract(false)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--ink3)" }}>✕</button>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                {[
                  ...
                  <div key={f.field}>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "var(--ink3)", marginBottom: 6, textTransform: "uppercase", letterSpacing: ".06em" }}>{f.label}</label>
                    {f.type === "select" ? (
                      ...
                      <input type={f.type || "text"} value={(newContract as any)[f.field]} onChange={e => setNewContract(prev => ({ ...prev, [f.field]: e.target.value }))}
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "var(--ink3)", marginBottom: 6, textTransform: "uppercase", letterSpacing: ".06em" }}>Scope of Work Description *</label>
                <textarea value={newContract.sowDescription} onChange={e => setNewContract(prev => ({ ...prev, sowDescription: e.target.value }))}
              ...
              <div style={{ marginBottom: 20, padding: "16px 18px", background: "var(--cream)", borderRadius: "var(--r)", border: "1px solid var(--border)" }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: "var(--navy)", marginBottom: 10 }}>Do you have a CPARS report for this contract?</div>
                <div style={{ fontSize: 12, color: "var(--ink3)", marginBottom: 12, lineHeight: 1.5 }}>CPARS (Contractor Performance Assessment Reporting System) reports are issued by federal agencies for government contracts. If you have one, it's the strongest form of past performance.</div>
              ...
              {/* Reference info — always shown */}
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: "var(--navy)", marginBottom: 4 }}>
                  {newContract.hasCPARS === false ? "Reference Contact (for PPQ)" : "Reference Contact"}
                </div>
                <div style={{ fontSize: 12, color: "var(--ink3)", marginBottom: 12, lineHeight: 1.5 }}>
                  {newContract.hasCPARS === false
                    ? "GovCert will email this person a PPQ form to complete online. Make sure to let them know it's coming."
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {[
                    ...
                    <div key={f.field}>
                      ...
                      <input type="text" value={(newContract as any)[f.field]} onChange={e => setNewContract(prev => ({ ...prev, [f.field]: e.target.value }))}
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                ...
                <button onClick={addNewContract} disabled={saving || !newContract.agencyName || !newContract.sowDescription}
                  style={{ padding: "10px 28px", background: "var(--gold)", border: "none", borderRadius: "var(--r)", color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer", boxShadow: "0 4px 16px rgba(200,155,60,.3)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
                    {contract.hasCPARS === true && (
                      <span style={{ padding: "3px 10px", borderRadius: 100, fontSize: 11, fontWeight: 500, background: contract.cparsUploaded ? "var(--green-bg)" : "var(--cream2)", color: contract.cparsUploaded ? "var(--green)" : "var(--ink3)" }}>
                        {contract.cparsUploaded ? "✓ CPARS" : "CPARS"}
                      </span>
                    )}
                      ...
                      {[
                        ...
                        <div key={f.field}>
                          ...
                          {f.type === "select" ? (
                            ...
                            <input type={f.type || "text"} value={(contract as any)[f.field]} onChange={e => updateContract(index, f.field, e.target.value)}
                              style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--border2)", borderRadius: "var(--r)", fontSize: 13, outline: "none", boxSizing: "border-box" as const, fontFamily: "'DM Sans', sans-serif" }} />
                          )}
                        </div>
                      ))}
                          ...
                          <div>
                            ...
                            {contract.cparsUploaded ? (
                              ...
                              <div>
                                ...
                                <button onClick={() => { cparsUploadTarget.current = String(index); fileInputRef.current?.click(); }}
                                  disabled={uploadingCPARS === String(index)}
                            ...
                            {contract.ppqStatus === "NOT_SENT" || contract.ppqStatus === "PENDING" ? (
                              <div>
                                <div style={{ fontSize: 11.5, color: "var(--ink3)", marginBottom: 10, lineHeight: 1.5 }}>
                                  GovCert will email a GSA-standard PPQ form to your reference. They complete it online — no PDF or printing required.
                                </div>
                                ...
                                {!contract.referenceEmail && (
                                  <div style={{ fontSize: 11, color: "var(--red)", marginTop: 6 }}>Add reference email below to send PPQ</div>
                                )}
                              </div>
                            ) : (
                              <div style={{ fontSize: 12, color: "var(--ink3)", lineHeight: 1.5 }}>
                                {contract.ppqStatus === "COMPLETED" ? "✓ Your reference has completed the PPQ." : "Waiting for your reference to complete the PPQ. GovCert will notify you when it's done."}
                              </div>
                            )}
                            <button onClick={() => updateContract(index, "hasCPARS", null)}
                              style={{ marginTop: 8, background: "none", border: "none", fontSize: 11, color: "var(--ink4)", cursor: "pointer", textDecoration: "underline" }}>
                    ...
                    <div style={{ borderTop: "1px solid var(--border)", paddingTop: 20 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: "var(--ink3)" }}>Required for each SIN — describes the work, methodology, and results achieved.</div>
                        </div>
                        <button onClick={() => generateNarrative(index)} disabled={generatingNarrative === (contract.id || String(index))}
                          style={{ padding: "8px 18px", background: "var(--navy)", border: "none", borderRadius: "var(--r)", color: "var(--gold2)", fontSize: 13, fontWeight: 500, cursor: "pointer", flexShrink: 0 }}>
                      ...
                      <textarea value={contract.narrative} onChange={e => updateContract(index, "narrative", e.target.value)}
                        placeholder="Describe the work performed, results achieved, methodology used, and how this project relates to your proposed GSA Schedule offering..."
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 32 }}>
            ...
            <a href={`/certifications/${params.id}`}
              style={{ padding: "12px 28px", background: "var(--gold)", border: "none", borderRadius: "var(--r)", color: "#fff", fontSize: 14, fontWeight: 500, cursor: "pointer", boxShadow: "0 4px 16px rgba(200,155,60,.35)", textDecoration: "none" }}>
              Save & Continue →
            </a>
          </div>
        </div>
