  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 48px" }}>
        ...
        {/* ── Tips & Disclaimer — shown while waiting or before first review ── */}
        {(reviewPending || (!review && !analyzing)) && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
            {[
              { icon: "📋", title: "Review everything before submitting", body: "GovCert's AI review is a tool to help you prepare — it is not a substitute for your own careful review. You are responsible for the accuracy of every statement in your submission." },
              { icon: "📄", title: "Keep backup documentation ready", body: "GSA Contracting Officers often request clarification within 30 days of submission. Have contract numbers, POC contact info, and financial documentation ready to respond quickly." },
              { icon: "⏰", title: "Plan for clarification rounds", body: "Most GSA MAS offers go through 1-3 clarification rounds. Each round has a deadline. Slow responses can result in administrative closure of your offer." },
              { icon: "🔒", title: "Verify SAM.gov before submitting", body: "Every field in eOffer must match your SAM.gov registration exactly. Check your UEI, CAGE, business name, and address. Any mismatch triggers a clarification notice." },
            ].map((tip, i) => (
              <div key={i} style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "var(--r)", padding: "16px 20px", boxShadow: "var(--shadow)" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>{tip.icon}</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", marginBottom: 4 }}>{tip.title}</div>
                    <div style={{ fontSize: 12, color: "var(--ink3)", lineHeight: 1.6 }}>{tip.body}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {(reviewPending || (!review && !analyzing)) && (
          <div style={{ background: "rgba(99,102,241,.04)", border: "1px solid rgba(99,102,241,.12)", borderRadius: "var(--r)", padding: "16px 20px", marginBottom: 20 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ fontSize: 16, flexShrink: 0 }}>⚠️</span>
              <div style={{ fontSize: 12, color: "var(--ink3)", lineHeight: 1.7 }}>
                <strong style={{ color: "var(--navy)" }}>Your submission is your responsibility.</strong> GovCert provides AI-powered analysis to help you prepare a stronger application, but it does not guarantee approval and cannot predict agency decisions. All content generated by GovCert should be reviewed carefully before submission. You are solely responsible for the accuracy, completeness, and truthfulness of your submission to the government. GovCert is not affiliated with the SBA, GSA, or any government agency. Consult a government contracting attorney or certified consultant before submitting.
              </div>
            </div>
          </div>
        )}
