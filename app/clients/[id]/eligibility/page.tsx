...
export default function AdminEligibilityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    try {
      const clientData = await apiRequest(`/api/clients/${clientId}`);
      setClient(clientData);
      setBusinessName(clientData.businessName || "");
      setEin(clientData.ein || "");

      try {
        const intake = await apiRequest(`/api/eligibility/${clientId}`);
        if (intake) {
  ...
  async function runAssessment() {
    ...
    try {
      ...
      router.push(`/clients/${clientId}?assessed=true`);
    } catch (err) {
      console.error(err);
      setAssessing(false);
    }
      ...
      <div style={{ width: 240, background: "var(--navy)", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
        ...
        <nav style={{ padding: "16px 12px", flex: 1 }}>
          {[
            ...
            { label: "Clients", icon: "\uD83D\uDC65", href: "/clients", active: true },
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        <div style={{ padding: "40px 48px", maxWidth: 860 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <a href={`/clients/${clientId}`} style={{ fontSize: 13, color: "var(--gold)", textDecoration: "none" }}>&larr; Back to {client?.businessName || "Client"}</a>
          </div>
          <div style={{ marginBottom: 32 }}>
            ...
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 36, color: "var(--navy)", fontWeight: 400, lineHeight: 1.1, marginBottom: 8 }}>
              {client?.businessName || "Client"} Eligibility Wizard
            </h1>
            <p style={{ fontSize: 14, color: "var(--ink3)", fontWeight: 300 }}>Complete each section to assess certification eligibility.</p>
          </div>
            ...
            {step === 4 && (
              <div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--ink4)", marginTop: 4 }}>Auto-detected from client profile or address.</div>
                </div>
              </div>
            )}
