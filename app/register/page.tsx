      ...
      <div style={{ position: "relative", zIndex: 2, padding: "40px 20px" }}>
            ...
            {[
              ...
              <div key={s.n} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: step >= s.n ? "var(--gold)" : "rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: step >= s.n ? "#fff" : "rgba(255,255,255,.3)", fontWeight: 600, transition: "all .3s" }}>
                    {step > s.n ? "✓" : s.n}
                  </div>
                  <span style={{ fontSize: 12, color: step >= s.n ? "rgba(255,255,255,.7)" : "rgba(255,255,255,.3)", transition: "all .3s" }}>{s.label}</span>
                </div>
                {i < 2 && <div style={{ width: 32, height: 1, background: step > s.n ? "var(--gold)" : "rgba(255,255,255,.15)" }} />}
              </div>
            ))}
          </div>
        </div>
        ...
        {step === 1 && (
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 36 }}>
              <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 38, color: "#fff", fontWeight: 400, marginBottom: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 32 }}>
              {PLANS.map(plan => (
                <div key={plan.id}
                  ...
                  style={{ background: "rgba(255,255,255,.04)", border: `2px solid ${selectedPlan === plan.id ? "var(--gold)" : "rgba(255,255,255,.08)"}`, borderRadius: 16, padding: "32px 28px", cursor: "pointer", transition: "all .2s", position: "relative", overflow: "hidden" }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = "var(--gold)", e.currentTarget.style.background = "rgba(255,255,255,.06)")}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = selectedPlan === plan.id ? "var(--gold)" : "rgba(255,255,255,.08)", e.currentTarget.style.background = "rgba(255,255,255,.04)")}>
                  ...
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, color: "#fff", fontWeight: 400, marginBottom: 4 }}>{plan.name}</div>
                  ...
                  <div style={{ marginBottom: 20, paddingBottom: 20, borderBottom: "1px solid rgba(255,255,255,.08)" }}>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 36, color: "var(--gold2)", fontWeight: 400, lineHeight: 1 }}>{plan.price}</div>
                  ...
                  {/* Select button */}
                  <button style={{ width: "100%", marginTop: 20, padding: "13px", background: "var(--gold)", border: "none", borderRadius: "var(--r)", color: "#fff", fontSize: 14, fontWeight: 500, cursor: "pointer", boxShadow: "0 4px 24px rgba(200,155,60,.3)", fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <div style={{ textAlign: "center", fontSize: 13, color: "rgba(255,255,255,.3)" }}>
              ...
              <a href="/login" style={{ color: "var(--gold2)", textDecoration: "none", fontWeight: 500 }}>Sign in</a>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ maxWidth: 500, margin: "0 auto" }}>
            ...
            <div style={{ background: "rgba(200,155,60,.1)", border: "1px solid rgba(200,155,60,.2)", borderRadius: 10, padding: "12px 16px", marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
              ...
              <button onClick={() => setStep(1)} style={{ background: "none", border: "none", color: "rgba(255,255,255,.4)", fontSize: 12, cursor: "pointer", textDecoration: "underline" }}>
                Change
              </button>
            </div>

            <div style={{ textAlign: "center", marginBottom: 28 }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, color: "#fff", fontWeight: 400, marginBottom: 6 }}>
                Create your account
              </h2>
              <p style={{ fontSize: 14, color: "rgba(255,255,255,.4)" }}>You'll verify your email after signing up</p>
            </div>

            <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.09)", borderRadius: 12, padding: "36px 32px", backdropFilter: "blur(12px)" }}>
              {error && (
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div>
                    ...
                    <input type="text" value={form.firstName} onChange={e => update("firstName", e.target.value)} required
                      placeholder="Jane"
                      style={{ width: "100%", padding: "11px 14px", background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", borderRadius: "var(--r)", fontSize: 14, color: "#fff", outline: "none", boxSizing: "border-box" as const, fontFamily: "'DM Sans', sans-serif" }} />
                  </div>
                  <div>
                ...
                <div style={{ marginBottom: 28 }}>
                  ...
                  <input type="password" value={form.confirmPassword} onChange={e => update("confirmPassword", e.target.value)} required
                    placeholder="........"
                    style={{ width: "100%", padding: "11px 14px", background: "rgba(255,255,255,.06)", border: `1px solid ${form.confirmPassword && form.password !== form.confirmPassword ? "var(--red)" : "rgba(255,255,255,.12)"}`, borderRadius: "var(--r)", fontSize: 14, color: "#fff", outline: "none", boxSizing: "border-box" as const, fontFamily: "'DM Sans', sans-serif" }} />
                  {form.confirmPassword && form.password !== form.confirmPassword && (
                    <div style={{ fontSize: 11, color: "var(--red)", marginTop: 4 }}>Passwords do not match</div>
                  )}
                </div>

                <div style={{ marginBottom: 20, padding: "12px 14px", background: "rgba(255,255,255,.04)", borderRadius: "var(--r)", fontSize: 12, color: "rgba(255,255,255,.35)", lineHeight: 1.6 }}>
        ...
        {step === 3 && (
          <div style={{ maxWidth: 480, margin: "0 auto" }}>
            <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.09)", borderRadius: 16, padding: "48px 40px", textAlign: "center", backdropFilter: "blur(12px)" }}>
              <div style={{ fontSize: 52, marginBottom: 20 }}>📬</div>
              <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, color: "#fff", fontWeight: 400, marginBottom: 10 }}>
                Check your email
              </h2>
              ...
              {/* What to expect */}
              <div style={{ background: "rgba(200,155,60,.08)", border: "1px solid rgba(200,155,60,.15)", borderRadius: 10, padding: "18px 20px", marginBottom: 28, textAlign: "left" as const }}>
              ...
              <a href="/login" style={{ display: "block", padding: "13px", background: "var(--gold)", borderRadius: "var(--r)", color: "#fff", fontSize: 14, fontWeight: 500, textDecoration: "none", boxShadow: "0 4px 24px rgba(200,155,60,.3)", marginBottom: 12 }}>
                Go to Login →
              </a>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,.25)" }}>
                Didn't receive the email? Check your spam folder or{" "}
                <button onClick={() => setStep(2)} style={{ background: "none", border: "none", color: "rgba(255,255,255,.4)", fontSize: 12, cursor: "pointer", textDecoration: "underline" }}>
                  try again
                </button>
              </div>
            </div>
